...
YMM4 v4.56.1.0（通常版／Lite）は別の保守系列（`.NET 10.0.11` 境界）として暫定候補に対応しています。導入・CLI・GUI起動まで検証済みですが、編集・再生・書き出し等の版固有動作は未検証のため、確認画面で了承した場合のみ導入されます。未知の更新版ZIPを選ぶときは、公式のファイル名（`YukkuriMovieMaker_v<version>[_Lite].zip`）を変更せずに選択してください。リネームしたZIPは公式assetとして確認できず導入できません。
