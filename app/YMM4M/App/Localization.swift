...
extension Loc {
    static let ja = Loc(
        ...
        bulkInstallFreeSpace: "選んだ元ZIPは移動・削除しません。途中で失敗しても同じボタンから再開でき、不完全な管理領域はRecoveryへ退避します。完了まで機種により1〜数時間と約10 GB以上の空き容量（LLVM toolchainの展開分を含む）が必要です。",
        ...
        usageStep4: "4. 完了後に「YMM4をMacで開く」を押します。YMM4の初回起動時はYMM4側の確認（更新確認など）が表示されることがあります。内容を確認して進めてください。",
        ...
        statusBulkStarted: "一括セットアップを開始します。不完全な管理領域を安全に退避し、runtimeとprefixを検証しています…終わるまでアプリを終了しないでください。",
    ...
    static let en = Loc(
        ...
        bulkInstallFreeSpace: "Your source ZIP is never moved or deleted. If a run is interrupted you can resume from the same button, and incomplete managed areas are moved to Recovery. Allow one to several hours (depending on your Mac) and about 10 GB of free space (including the extracted LLVM toolchain).",
        ...
        usageStep4: "4. When it finishes, press “Open YMM4 on the Mac”. On YMM4's first launch, YMM4 itself may show confirmation dialogs (such as an update check). Review them and proceed.",
        ...
        statusBulkStarted: "Starting the one-shot setup. Moving any incomplete managed area aside safely and verifying the runtime and prefix… Do not quit the app until it finishes.",
